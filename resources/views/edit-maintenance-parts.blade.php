...
<div class="col-3 mb-5">
    <input class='form-control text-center fw-bold' type="text" readonly value='{{$maintenance->elevator_type}}' style="pointer-events: none">
</div>
      ...
      @foreach ($partsOG as $part)
        ...
        <div class="d-flex align-items-center justify-content-start mb-5">
          ...
          @if (in_array($part->id, $partsArray))
            ...
            @if ($type == 'bill')
              ...
              <div class="d-inline-block">
                ...
                max='{{ ($part->quantity + ($parts->where('part_id', $part->id)->first()->quantity * $maintenance->elevator_count)) / $maintenance->elevator_count  }}' 
            ...
            @else
              ...
              <div class="d-inline-block">
                <label class='d-block fs-11'>الكمية</label>
                <input class='form-control parts--input' type="number" step='1' min='0' 
                max='{{ $part->quantity / $maintenance->elevator_count}}' 
                name="part_quantity[{{$part->id}}][]" 
                value='{{$parts->where('part_id', $part->id)->first()->quantity}}' {{ ($part->quantity == 0 && $parts->where('part_id', $part->id)->first()->quantity == 0 ) ? 'readonly' : ''}}>
              </div>
          ...
          @else
            ...
            <div class="d-inline-block">
              <label class='d-block fs-11'>الكمية</label>
              <input class='form-control parts--input' type="number" step='1' min='0' max='{{$part->quantity / $maintenance->elevator_count}}' name="part_quantity[{{$part->id}}][]" id="" {{ ($part->quantity > 0) ? '' : 'readonly'}}>
            </div>
